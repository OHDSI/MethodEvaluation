...
#' @title Compute minimal detectable relative risk (MDRR)
#'
#' @description
...
#'
#' @details
#' Computes the MDRR using simple power-calculations using person-level statistics stratified by age and gender.
#'  
...
#' 
#' @return A data frame containing the MDRRs for the given exposure-outcome pairs.
#' 
#' @examples \dontrun{
...
#'   mdrrs <- computeMdrr(connectionDetails, "cdm_truven_mdcr" exposureOutcomePairs, outcomeTable = "condition_era")
#' }
#' @export
computeMdrr <- function(connectionDetails, 
  ...
  mdrr <- data.frame(exposureConceptId=mdrr$DRUG_CONCEPT_ID,outcomeConceptId=mdrr$CONDITION_CONCEPT_ID,expectedCount=mdrr$EXPECTED_COUNT,mdrr=mdrr$MDRR)
...
#' 
#' @return A subset of the data object.
#' 
#' @export
filterOnMdrr <- function(data, mdrr, threshold = 1.25){
  subset <- subset(mdrr, mdrr <= threshold, select = c("exposureConceptId","outcomeConceptId"))
  return(merge(data, subset, by = c("exposureConceptId","outcomeConceptId")))
}
